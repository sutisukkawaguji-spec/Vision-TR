...
const dashboardState = { groupId: new URLSearchParams(location.search).get('workGroup') || '', year: 'all', graph: 'bar', fieldKey: '' };
...
function render() {
  ...
  renderPeople(features); renderStatus(); renderTrend(features);
...
function renderChart(items) {
  ...
  document.querySelectorAll('.chart-type').forEach(button => button.className=`chart-type flex-1 text-sm font-bold ${button.dataset.chart===dashboardState.graph?'bg-emerald-600 text-white':'text-slate-600'}`);
}
...
function renderStatus() { const labels={waiting:'รอดำเนินการ',navigating:'กำลังเดินทาง',checking:'กำลังตรวจ',done:'เสร็จสิ้น',problem:'มีปัญหา'};document.getElementById('plot-status').innerHTML=Object.entries(labels).map(([key,label])=>`<div class="flex justify-between text-sm py-1"><span>${label}</span><b>${dashboardRecords.filter(record=>record.status===key).length}</b></div>`).join(''); }
