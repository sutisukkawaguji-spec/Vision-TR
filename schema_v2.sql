...
begin
  ...
  select * into member_profile from public.profiles where lower(user_code) = lower(trim(member_code));
  ...
  update public.profiles set team_id = caller_profile.team_id, updated_at = now() where profiles.id = member_profile.id
    returning profiles.id, profiles.email, profiles.display_name, profiles.user_code into id, email, display_name, user_code;
  return next;
end;
$$;
